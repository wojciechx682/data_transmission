...
int main()
{
    ...
    float* Za = new float[N];
        float* Za_dB = new float[N];
    float* Zp = new float[N];
    float* Zf = new float[N];
    
    float Ka = 25; // współczynnik głębokości modulacji
    float Kp = 4 * M_PI; // ZMIENIĆ
    

    float Fm = 1000; // częst. sygnał modulującego  = 100 Hz
        //float Fn = 10000; // częst. sygnał modulującego  = 100 Hz
    float Fn = 25000; // ZWIĘKSZYĆ

    float Kf = 5 * M_PI; // >= 0.75 * Fm ( Fn (?) )
    ...
    float* M_dec = new float[N];
        float* max_freq = new float[N];
        float* min_freq = new float[N];
    ...
    // find max and min value in array (frequency)

    for (int k = 0; k < N; k++)
    {       

        if (max_freq[0] < max_freq[k])
            max_freq[0] = max_freq[k];

        if (min_freq[0] > min_freq[k])
            min_freq[0] = min_freq[k];
    ...
    //cout << "\n max value in array = " << max_freq[0];
    //cout << "\n min value in array = " << min_freq[0];
